...
- **잠금화면 스냅샷 동기화** — 배경화면을 적용할 때 정적 프레임을 추출해 macOS 데스크톱/잠금화면 배경으로 같이 맞출 수 있습니다.
- **인터랙티브 이미지 오브젝트** — 배경화면 위에 앨범 커버나 원하는 이미지를 얹고, 편집 모드에서 클릭하거나 드래그해 위치를 맞출 수 있습니다.
- **Live2D / Cubism 웹 오브젝트** — 라이선스가 있는 로컬 Live2D 웹 번들을 넣으면 배경화면 위에 투명한 인터랙티브 WebKit 오브젝트로 렌더링합니다. 로컬 전용 스킴으로 샌드박스되며 외부 네트워크는 차단됩니다.
- **Steam Workshop 연동** — 창작마당 페이지·아이템 열기, 이미 받아 둔 `steamapps/workshop/content/431960` 폴더 가져오기, Wallpaper Engine을 보유한 계정으로 SteamCMD 로그인 다운로드까지 지원합니다. Steam을 우회하거나 파일을 재배포하지 않습니다.
...
- **`Add Live2D Web Object...`** — 로컬 Live2D / Cubism 웹 번들을 복사해 투명한 인터랙티브 WebKit 오브젝트로 렌더링합니다(아래 **Live2D 웹 오브젝트** 섹션 참고).
...
애니메이션 **Live2D / Cubism** 캐릭터를 영상 배경화면 위에 인터랙티브 오브젝트로 올릴 수 있습니다.

- **`Interactive Objects` → `Add Live2D Web Object...`**를 열고, Live2D 웹 진입 파일(Cubism 런타임과 모델 에셋이 든 `index.html`)이 있는 폴더를 선택합니다.
- 번들은 현재 프로젝트로 복사되어 **페이지와 캔버스 배경이 투명한** 상태로 WebKit에서 렌더링되므로, 배경화면 위에 캐릭터만 보입니다.
- **앱 로컬 전용 URL 스킴**으로 불러오고 **모든 외부 네트워크 리소스를 차단**해, 완전히 오프라인·샌드박스 상태로 동작합니다.
- **`Edit / Interact With Objects`**로 캐릭터를 드래그해 배치하고, `Esc`를 누르거나 바깥을 클릭하면 배경화면이 다시 데스크톱 아이콘 뒤로 돌아갑니다.
- **`Audio Responsive`**와 함께 쓰면 오디오 반응형 장면 위에 캐릭터를 얹을 수 있습니다.

> ⚠️ Live2D / Cubism 런타임은 앱에 **포함되어 있지 않습니다.** 사용 권한이 있는 로컬 런타임 파일과 모델 에셋을 직접 넣어야 합니다.

---

## 🔒 잠금화면 스냅샷

macOS는 일반 앱 창을 잠금화면 위에 계속 표시하도록 허용하지 않습니다. 그래서 Wallpaper Engine Mac은 수동으로 배경화면을 적용할 때, 어울리는 정적 잠금화면 이미지를 함께 설정할지 묻습니다. 수락하면 영상, 이미지, scene fallback 또는 preview에서 PNG 스냅샷을 추출하고, 그 이미지를 라이브 레이어 아래의 macOS 데스크톱 배경과 잠금화면 캐시에 적용합니다. 캐시 갱신에는 macOS가 관리자 암호를 요청할 수 있습니다.

나중에 다시 맞추고 싶으면 메뉴바 앱의 **`Set Current Snapshot as Lock Screen`**을 실행하면 됩니다.
